...
void Sbus::SbusRead(HardwareSerial& uart)
{
    ...
    ch[0]  = ((buffer[1] | buffer[2]<<8)&0x07FF);// 捨て用
    ch[1]  = ((buffer[2]>>3 | buffer[3]<<5)&0x07FF);//ch1 エルロン
    ch[2]  = ((buffer[3]>>6 | buffer[4]<<2| buffer[5]<<10)&0x07FF);//ch2 エレベーター
    ch[3]  = ((buffer[5]>>1 | buffer[6]<<7)&0x07FF);//ch3 推力L
    ch[4]  = ((buffer[6]>>4 | buffer[7]<<4)&0x07FF);//ch4 横力板
    ch[5]  = ((buffer[7]>>7 | buffer[8]<<1| buffer[9]<<9)&0x07FF);//ch5 ラダー
    ch[6]  = ((buffer[9]>>2 | buffer[10]<<6)&0x07FF);//ch6 推力R 
    ch[7]  = ((buffer[10]>>5 | buffer[11]<<3)&0x07FF);//ch7
    ch[8]  = ((buffer[12] | buffer[13]<<8)&0x07FF);//ch8 ロール
